    ...
    <div style={{ background: T.bg, color: T.ink, minHeight: '100vh', fontFamily: "'Geist', sans-serif" }}>
      ...
      <div key={theme} style={{ display: 'flex', minHeight: '100vh' }}>
        ...
        <main style={{ flex: 1, padding: '48px 40px', maxWidth: 1600, margin: '0 auto', width: '100%' }}>
          ...
          {view === 'changes' && <ChangesView campaigns={campaigns} periods={periods} stockRowsPO2={stockRowsPO2} stockRowsPO3={stockRowsPO3} stockMapPO2={stockMapPO2} stockMapPO3={stockMapPO3} />}
...
function ChangesView({ campaigns, periods, stockRowsPO2, stockRowsPO3, stockMapPO2, stockMapPO3 }) {
  const [snapshot, setSnapshot] = useState(null);
  const [compareSource, setCompareSource] = useStoredState('changes.compareSource', 'campaign');
  ...
  // Auto-pick first available item when state is empty
  useEffect(() => {
    if (compareSource === 'campaign' && !compareWithId && campaigns.length > 0) {
      setCompareWithId(campaigns[0].id);
    }
    if (compareSource === 'period' && !comparePeriodId && periods?.length > 0) {
      setComparePeriodId(periods[0].id);
    }
  }, [campaigns, periods, compareSource, compareWithId, comparePeriodId]);

  const compareWith = compareSource === 'period'
    ? buildPeriodCompareWith(comparePeriodId, campaigns, periods)
    : campaigns.find(c => c.id === compareWithId);
  ...
  return (
    <div className="fade-up">
      ...
      {campaigns.length === 0 ? (
        ...
        <div>
          {/* Source selector */}
          <div style={{ marginBottom: 16, padding: '14px 16px', background: T.bgEl, border: `1px solid ${T.line}`, borderRadius: 8 }}>
            <div className="mono" style={{ fontSize: 10, letterSpacing: '0.12em', color: T.inkMute, textTransform: 'uppercase', marginBottom: 10 }}>
              Comparar com
            </div>
            {/* Toggle: Campanha (Excel) vs Período */}
            <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
              {[
                { id: 'campaign', label: 'Campanha (Excel)' },
                { id: 'period', label: 'Período / Campanha' },
              ].map(s => (
                <button key={s.id} onClick={() => setCompareSource(s.id)} style={{
                  padding: '5px 12px', fontSize: 12, fontWeight: 500, borderRadius: 5,
                  border: `1px solid ${compareSource === s.id ? T.accent : T.line}`,
                  background: compareSource === s.id ? T.accentSoft : 'transparent',
                  color: compareSource === s.id ? T.accent : T.inkSoft,
                  cursor: 'pointer',
                }}>{s.label}</button>
              ))}
            </div>
            {compareSource === 'campaign' ? (
              campaigns.length === 0 ? (
                <div style={{ fontSize: 13, color: T.inkMute }}>Nenhum Excel carregado ainda.</div>
              ) : (
                <select value={compareWithId || ''} onChange={e => setCompareWithId(e.target.value)}
                  style={{ width: '100%', padding: '7px 10px', fontSize: 13, color: T.ink, background: T.paper, border: `1px solid ${T.line}`, borderRadius: 5 }}>
                  {campaigns.map(c => (
              ...
              !periods?.length ? (
                <div style={{ fontSize: 13, color: T.inkMute }}>Nenhum período criado ainda.</div>
              ) : (
                <>
                  <select value={comparePeriodId || ''} onChange={e => setComparePeriodId(e.target.value)}
                    style={{ width: '100%', padding: '7px 10px', fontSize: 13, color: T.ink, background: T.paper, border: `1px solid ${T.line}`, borderRadius: 5 }}>
                    {periods.map(p => {
                      const count = campaigns.filter(c => c.periodId === p.id).reduce((s, c) => s + (c.rows?.length || 0), 0);
                      return <option key={p.id} value={p.id}>{p.name} ({count} produtos)</option>;
                    })}
                  </select>
                  {comparePeriodId && (() => {
                    const camps = campaigns.filter(c => c.periodId === comparePeriodId);
          ...
          <DropZone
            label="Arrasta o novo ficheiro da campanha"
        ...
        <ChangesReport
          ...
          setComparePeriodId={setComparePeriodId}
          stockRowsPO3={stockRowsPO3}
        />
      )}
    </div>
...
function ChangesReport({ snapshot, compareWith, diff, filter, setFilter, filterAveiro, setFilterAveiro, search, setSearch, filteredItems, onClear, onExport, campaigns, periods, compareSource, setCompareSource, compareWithId, setCompareWithId, comparePeriodId, setComparePeriodId, stockRowsPO3 }) {
  if (!diff) return null;
      ...
      <div style={{
        ...
        <div style={{ flex: '1 1 auto' }}>
          ...
          <div style={{ display: 'flex', gap: 4, marginBottom: 6 }}>
            {[{ id: 'campaign', label: 'Excel' }, { id: 'period', label: 'Período' }].map(s => (
              <button key={s.id} onClick={() => setCompareSource(s.id)} style={{
                padding: '3px 9px', fontSize: 11, fontWeight: 500, borderRadius: 4,
                border: `1px solid ${compareSource === s.id ? T.accent : T.line}`,
                background: compareSource === s.id ? T.accentSoft : 'transparent',
                color: compareSource === s.id ? T.accent : T.inkSoft,
                cursor: 'pointer',
          ...
          {compareSource === 'campaign' ? (
            <select value={compareWithId || ''} onChange={e => setCompareWithId(e.target.value)}
              style={{ padding: '4px 8px', fontSize: 13, fontWeight: 500, color: T.ink, background: T.paper, border: `1px solid ${T.line}`, borderRadius: 4, maxWidth: 320 }}>
