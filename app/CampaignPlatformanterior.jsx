...
async function cloudFetchAllCampaigns() {
  ...
  const { data, error } = await supabase
    .from('campaigns')
    .select('id, user_id, period_id, campaign_key, name, headers, rows, floors, uploaded_at, updated_at, created_by, updated_by')
...
async function cloudUpsertCampaign(campaign, userId) {
  if (!supabase) return { ok: false };
  const isNew = !campaign.id || typeof campaign.id === 'number';
  const payload = {
    user_id: campaign.user_id || userId,
    period_id: campaign.periodId || null,
    campaign_key: campaign.key,
    name: campaign.name,
    headers: campaign.headers || [],
    rows: campaign.rows || [],
    floors: campaign.floors,
...
function MainApp({ onLogout, user, theme, toggleTheme, setTheme }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 4. Migrate local-only periods to cloud
      let migratedCount = 0;
      for (const p of localOnlyPeriods) {
      ...
      // 5. Migrate local-only campaigns to cloud
      for (const c of localOnlyCampaigns) {
        if (!c.key) continue;
        const campaignWithUser = { ...c, user_id: c.user_id || user.id };
      ...
      const [finalPeriods, finalCampaigns] = await Promise.all([
        cloudFetchAllPeriods(),
        cloudFetchAllCampaigns(),
      ]);
      if (cancelled) return;

      // 7. Replace local state with cloud (cloud is now source of truth)
        ...
        <main style={{ flex: 1, padding: '48px 40px', maxWidth: 1600, margin: '0 auto', width: '100%' }}>
          ...
          {migrationStatus && (
            <div style={{
              ...
              background: '#E5F4E5', color: '#2E5E2A',
              border: '1px solid #B8DDB6', borderRadius: 8,
              ...
              <button onClick={() => setMigrationStatus(null)} style={{
                ...
                color: '#2E5E2A', border: 'none', cursor: 'pointer',
              }}><X size={12} /></button>
            </div>
          )}
          ...
          {view === 'changes' && <ChangesView campaigns={campaigns} stockRowsPO2={stockRowsPO2} stockRowsPO3={stockRowsPO3} stockMapPO2={stockMapPO2} stockMapPO3={stockMapPO3} />}
    ...
    <div className="fade-up">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 32 }}>
        ...
        <div style={{ padding: 16, background: T.bgEl, border: `1px solid ${T.line}`, borderRadius: 10 }}>
                ...
                return (
                  <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    <label style={{
                      ...
                      border: `1px solid ${isPrimary ? T.accent : 'transparent'}`,
                    ...
                    >
                      ...
                      />
                      <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: 12, color: T.ink, fontWeight: isPrimary ? 500 : 400 }}>
                        {c.name}
                      </span>
                      <span style={{ opacity: 0.6, fontSize: 10, color: T.inkMute }}>{c.itemCount}</span>
                      {slotCount > 0 && (
                        <span style={{
          ...
          {activeIds.length > 1 && (
            <div style={{ marginTop: 8, padding: '6px 8px', fontSize: 10, color: T.accent, background: T.accentSoft, borderRadius: 4, lineHeight: 1.4 }}>
              {activeIds.length} campanhas selecionadas. Listagem combinada. As alterações de plano aplicam-se à primária ({primaryCampaign?.name?.slice(0, 30) || '—'}).
            </div>
          )}
        </div>
        <div style={{ padding: 16, background: T.bgEl, border: `1px solid ${T.line}`, borderRadius: 10 }}>
    ...
    <div>
      ...
      <div className="print-area" style={{ background: T.bgEl, border: `1px solid ${T.line}`, borderRadius: 10, overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, minWidth: 1280, tableLayout: 'fixed' }}>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={14} style={{ padding: 32, textAlign: 'center', color: T.inkMute, fontSize: 13 }}>
...
const ProductRow = React.memo(function ProductRow({ product, zebra, floors, onSetPrimaryZone, onAssignMore }) {
  const p = product;
...
// Floor Planner
// ─────────────────────────────────────────────────────────────────────────
function FloorPlanner({ floor, editZones, activeCampaign, candidates, onAddZone, onRenameZone, onDeleteZone, onAddSlot, onUpdateSlot, onDeleteSlot }) {
  return (
    <section>
      <div style={{
...
// Zone Block
// ─────────────────────────────────────────────────────────────────────────
function ZoneBlock({ zone, color, editZones, activeCampaign, candidates, onRename, onDelete, onAddSlot, onUpdateSlot, onDeleteSlot }) {
  ...
  return (
    <div style={{ background: T.bgEl, border: `1px solid ${T.line}`, borderRadius: 8, overflow: 'hidden' }}>
      <header style={{
        ...
        <span className="mono" style={{ fontSize: 10, opacity: 0.8 }}>{zone.slots.length} produtos</span>
        {editZones && (
          <>
...
// ─────────────────────────────────────────────────────────────────────────
function ChangesView({ campaigns, stockRowsPO2, stockRowsPO3, stockMapPO2, stockMapPO3 }) {
  ...
  // Auto-pick first campaign when one becomes available
  useEffect(() => {
    if (!compareWithId && campaigns.length > 0) {
      setCompareWithId(campaigns[0].id);
    }
  }, [campaigns, compareWithId]);

  const compareWith = campaigns.find(c => c.id === compareWithId);
  ...
  const handleFile = async (file) => {
    ...
    setSnapshot({
      filename: file.name,
      uploaded: new Date(),
      headers, rows, itemCount: rows.length,
    });
  };
  ...
  return (
    <div className="fade-up">
      <Header
        eyebrow="Comparação"
        title="Alterações"
        subtitle="Carrega um snapshot novo da campanha (com todos os produtos atuais) e o sistema cruza com a campanha já carregada para mostrar o que foi adicionado, removido ou alterado nos preços."
      />
      ...
      {campaigns.length === 0 ? (
        <div style={{
          ...
          <div style={{ fontSize: 16, fontWeight: 500, marginBottom: 6 }}>Sem campanha base para comparar</div>
          <div style={{ fontSize: 13, color: T.inkMute }}>
        ...
        <div>
          <div style={{ marginBottom: 16, padding: '12px 16px', background: T.bgEl, border: `1px solid ${T.line}`, borderRadius: 8, display: 'flex', alignItems: 'center', gap: 12 }}>
            <span className="mono" style={{ fontSize: 11, color: T.inkMute, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
              Comparar com:
            </span>
            <select
              value={compareWithId || ''}
              ...
              style={{
                flex: 1, padding: '6px 10px', fontSize: 13, color: T.ink,
                background: T.paper, border: `1px solid ${T.line}`, borderRadius: 4,
              }}
            >
              {campaigns.map(c => (
                <option key={c.id} value={c.id}>{c.name} ({c.itemCount} produtos)</option>
              ))}
            </select>
          </div>
          <DropZone
            label="Arrasta o ficheiro novo da campanha"
            hint="snapshot completo com EAN, descrição, preços (.xlsx, .xls, .csv)"
            accept=".xlsx,.xls,.csv"
            onFile={handleFile}
...
function ChangesReport({ snapshot, compareWith, diff, filter, setFilter, search, setSearch, filteredItems, onClear, onExport, campaigns, compareWithId, setCompareWithId }) {
  if (!diff) return null;
    ...
    <div>
      ...
      <div style={{
        ...
        <div style={{ flex: '1 1 auto' }}>
          <div className="mono" style={{ fontSize: 9, letterSpacing: '0.12em', color: T.inkMute, textTransform: 'uppercase', marginBottom: 4 }}>
            Snapshot novo
          </div>
          <div style={{ fontSize: 13, fontWeight: 500 }}>{snapshot.filename}</div>
          <div style={{ fontSize: 11, color: T.inkMute }}>{snapshot.itemCount} produtos · carregado agora</div>
        </div>
        ...
        <div style={{ flex: '1 1 auto' }}>
          ...
          <select
            ...
            onChange={e => setCompareWithId(Number(e.target.value))}
            style={{
              padding: '4px 8px', fontSize: 13, fontWeight: 500, color: T.ink,
              background: T.paper, border: `1px solid ${T.line}`, borderRadius: 4,
              maxWidth: 320,
            }}
          >
          ...
          <div style={{ fontSize: 11, color: T.inkMute, marginTop: 2 }}>{compareWith?.itemCount} produtos</div>
        </div>
        <div style={{ display: 'flex', gap: 6, marginLeft: 'auto' }}>
          ...
          <button onClick={onClear} style={{
            padding: '8px 12px', fontSize: 12, background: 'transparent', color: T.inkSoft,
            border: `1px solid ${T.line}`, borderRadius: 6,
          }}>Novo snapshot</button>
        </div>
      </div>

      {/* Filters */}
      <div style={{ display: 'flex', gap: 8, marginBottom: 12, alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 12px', background: T.bgEl, border: `1px solid ${T.line}`, borderRadius: 6, flex: '1 1 280px', minWidth: 240 }}>
          <Search size={14} style={{ color: T.inkMute }} />
